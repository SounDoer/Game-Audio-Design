...
export function loadStemOrder(repoRoot) {
  const orderPath = path.join(repoRoot, 'slides', 'deck-order.txt');
  ...
  const legacySlides = path.join(repoRoot, 'slides', 'slides.md');
...
export function buildDeckEntryMarkdown(repoRoot, stem, deckTitle) {
  const headerPath = path.join(repoRoot, 'slides', 'deck-entry-header.in.yaml');
