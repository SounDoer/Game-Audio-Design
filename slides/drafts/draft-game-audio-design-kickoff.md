...
| 5 | 音频设计师需要从项目中获取哪些关键信息 | 需要哪些项目信息？ | 游戏玩法（核心循环/反馈节点）→ 美术风格（色调/材质/空间感）→ 世界观/叙事（时代/科技/文化坐标）；三类信息按重要程度排序，各栏标注对应的音频设计推断方向 | — | `header-body`，三栏 `grid-cols-3`，逐栏 `v-click` | 具体内容待用户确认 | confirmed |
...
| 20 | 三个层面的命名规范 | 命名规范 | 三栏：① 项目层面（地图/角色/物件的中英文对照与开发代码名）；② 资产层面（字段体系，举例枪声/Foley/语音）；③ 工程结构层面（目录结构、中间件嵌套、总线结构，与资产命名的关联） | — | `header-body`，三栏 `grid-cols-3`，逐栏 `v-click` | 具体示例待用户确认 | confirmed |
| 21 | 定义从 DAW 到引擎的标准工作流 | DAW → 中间件 → 引擎 | 三个环节：① DAW 侧（源文件管理、导出规范）；② 中间件侧（导入、Event 结构、参数配置）；③ 引擎侧（集成方式、运行时验证） | 用户将提供示例图 | `header-body`，三栏或图文组合 | — | confirmed |
| 22 | 了解完整的信号链路，快速定位问题出在哪个环节 | 信号流程：从 WAV 到听音设备 | 信号链路：WAV 资产 → Conversion（中间件导入转换）→ Actor-Mixer → Audio Bus → Audio Device → 终端听音设备；意义：理解信号流程是为了在出问题时能快速定位到具体环节，而不是靠猜测排查 | — | `header-body`，`<v-clicks>` 列表，可配信号链路示意图 | — | confirmed |
| 23 | 资产响度规范 | 资产响度规范 | TBD | — | TBD | — | needs-user |
| 24 | 混音切片参考 | 混音切片参考 | TBD | — | TBD | — | needs-user |
| 25 | Deck 结束页 | Thanks & 联系方式 | — | — | `end` | — | confirmed |
