...
- `slides/pages/ai-in-game-audio-design-2026.md`: missing
- `slides/public/ai-in-game-audio-design-2026/`: missing
- 已有来源文档:
  - `website/docs/...`: none
  - 其他参考: none
...
3. AIGC 方向一：语音 Voice（约 12 分钟）
   ...
   - 案例:
     - 代表工具 / 案例方向（待核验）: ElevenLabs 作为英文 AI 语音生成典型；Qwen 系列 TTS 模型作为国内典型，由用户补充具体模型和案例。
...
| S1-G2 AI 语音：游戏对白生产流程 | 说明 AI 语音在角色配音、临时对白和本地化中的使用范式 | ElevenLabs 作为英文 AI 语音生成典型；Qwen 系列 TTS 模型作为国内典型，由用户补充具体模型和案例；聚焦角色声音探索、临时配音、版本化对白、本地化；Replica Studios 不再作为代表候选 | confirmed |
...
| S2-G9 彩蛋：这套 slides 也是 AI 协作完成的 | 说明前面的协作范式不只适用于写代码，也适用于内容创作、课程准备和个人项目 | 展开这套 slides 如何与 AI 协作完成：如何写和使用 skills、如何指定 A/B/C 阶段流程、如何界定设计规范和写作边界、如何用 draft / slide groups / page plan 维持上下文与 SSOT。重点是让学生看到：即使不写代码，也可以把同样的协作思路用于自己的实际需求 | confirmed |
...
| 3 | 进入 AI 语音主题 | AI Voice：游戏对白生产流程 | 角色声音探索、临时对白、版本化对白、本地化 | 工具 logo 或文字占位 | section / header-body | 先强调不是猎奇 voice cloning，而是对白生产流程 | confirmed |
| 4 | 展示英文 AI 语音典型 | ElevenLabs：英文 AI 语音生成典型 | 高知名度、质量表现、角色声音探索、临时配音 | TODO: ElevenLabs 官网 / demo 截图 | tool-focus | 需要核验可展示材料和最新功能 | placeholder |
| 5 | 展示国内 AI 语音典型 | Qwen TTS：国内语音模型案例 | 国内模型进展、中文语音、可能的角色对白和本地化用途 | TODO(user): Qwen 系列 TTS 模型材料 | tool-focus | 等用户补充具体模型、链接、案例 | needs-user |
| 6 | 收束语音部分 | AI 语音真正改变了什么？ | 能加速占位、试音、本地化探索；仍要处理表演、授权、一致性 | 可用简短对比图 | summary | 连接到后面的 AIGC 通用风险 | confirmed |
| 7 | 进入 AI 音乐主题 | AI Music：从正式配乐到方向探索 | 本节只讲 Suno；重点是 mood、reference、temp music | Suno logo 或截图占位 | section / header-body | 明确不展开 Udio / Stable Audio | confirmed |
| 8 | 展示 Suno 的使用范式 | Suno：快速生成音乐方向 | mood board、reference、temp music、风格沟通 | TODO: Suno 公开材料 / demo 截图 | tool-focus | 需要避免把 Suno 讲成互动音乐系统替代品 | placeholder |
| 9 | 说明 AI 音乐边界 | 生成一首歌，不等于设计互动音乐 | loop、分层、转场、状态、混音、授权仍需人判断 | 可用“歌曲 vs 互动音乐系统”双栏 | comparison | 强调游戏音乐设计不是单个音频文件 | confirmed |
| 10 | 进入 AI 音效主题 | AI Sound Effects：文字与视频两条入口 | text-to-audio；video-to-audio / 视听生成 | 工具地图占位 | section / header-body | 素材库 AI 化后移到 AI 工具部分 | confirmed |
| 11 | 展示 text-to-audio | ElevenLabs Sound Effects：用文字生成音效 | prompt、短音效、foley、ambience、game SFX | TODO: ElevenLabs SFX 截图 / demo | tool-focus | 讲产品化程度相对高的音效生成入口 | placeholder |
| 12 | 展示 video-to-audio 前沿 | Video-to-audio：给画面自动生成声音 | MMAudio、PrismAudio；研究 / 开源 / 前沿方向 | TODO: MMAudio / PrismAudio demo 截图 | tool-focus | 谨慎表述：不是成熟主流商用工具 | placeholder |
| 13 | 补充视听一体生成趋势 | Seedance 2.0：native audio-video generation | 视频生成不再总是 silent，视听一体生成成为趋势 | TODO: Seedance 2.0 公开材料 | trend / case | 区分它和纯 video-to-audio 工具 | placeholder |
...
| 16 | 转入 AI 工具 | AI Tools：不是生成声音，而是改变工作流 | 找素材、修素材、做实现 | 简单三栏图 | section / map | 明确和 AIGC 区分 | confirmed |
...
| 26 | 阶段一案例 | 从需求开始：为什么我要做 audiometer？ | 我的需求；现有工具无法满足；开始 vibe coding | TODO(user): audiometer 早期材料 | case-story | 用户补充具体需求、痛点和早期截图 | needs-user |
| 27 | 阶段一体验 | 一问一答 vibe coding | 想法第一次能落地；错了重来、还是不对、再让它改 | TODO(user): 梗图 | quote / meme | 用梗图总结早期体验与局限 | needs-user |
...
| 31 | Audiometer 成品展示 | 这个工具现在变成了什么？ | 网站、截图、核心功能、从想法到产品形态的演进 | TODO(user): 网站 + 截图 | showcase | 用户提供网址、截图、功能点 | needs-user |
| 32 | 人在 AI 协作中的价值 | 工程决策 vs 审美决策 | 工程：目标、边界、架构、验收；审美：风格、取舍、体验、品味 | 双栏图 | two-column | 这是个人经验的观点收束 | confirmed |
| 33 | 彩蛋 | 这套 slides 也是 AI 协作完成的 | skills、A/B/C 流程、draft、slide groups、page plan、SSOT | 可用当前 draft 截图 | meta / case | 说明不写代码也能复用协作方法 | confirmed |
| 34 | 结尾 statement | 这套 slides 明年可能就要重写 | 工具会变，案例会变；真正带走的是判断标准、协作方法和验证意识 | 纯文字 | statement | 全课收束 | confirmed |

## Media Assets

| Need | Existing asset | Placeholder | Status |
|------|----------------|-------------|--------|
| 语音 AI 工具 / 案例截图 |  | TBD: 核验 ElevenLabs 公开材料；用户补充 Qwen 系列 TTS 模型与案例材料 | missing |
| 音乐 AI 工具 / 案例截图 |  | TBD: 核验 Suno 公开材料 | missing |
...
| Audiometer 成品展示素材 |  | TBD: 用户提供网站、截图、核心功能点 | missing |
| Slides AI 协作过程截图 |  | TBD: 可使用 draft、skill、page plan 或对话流程截图 | missing |
