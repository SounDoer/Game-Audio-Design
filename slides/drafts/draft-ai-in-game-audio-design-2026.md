...
| 25 | 阶段一体验 | 一问一答 vibe coding | 想法第一次能落地；但缺少结构、标准和验证时，很容易进入“错了重来、还是不对、再让它改”的低效循环 | 可选：梗图；没有素材时用 statement 或对话气泡表现 | quote / meme | 梗图不是必需素材，重点是把早期兴奋感和失控感讲清楚 | confirmed |
| 26 | 阶段二 | 从聊天，到理解 agents 与 skills | Claude、Cursor；brainstorming / grill-me、TDD / verification、code review、docs / slides authoring | 可用 skills 分类图 | process / list | 讲 skills 是流程设计，不是工具清单 | confirmed |
