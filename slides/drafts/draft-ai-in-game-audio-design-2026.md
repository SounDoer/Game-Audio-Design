...
| 26a | 补充协作原则案例 | Karpathy-Inspired Guidelines | 左侧合并说明 `andrej-karpathy-skills` 的来源、问题分类与四条原则；右侧展示完整 `CLAUDE.md` guideline 原文 | 官方中文 README：https://github.com/multica-ai/andrej-karpathy-skills/blob/main/README.zh.md；右侧代码块为用户提供的 `CLAUDE.md` 内容 | principle / text + full markdown source | 放在理解 agents 与 skills 之后，用它说明真正的改进来自协作协议和验收标准，而不只是换模型 | confirmed |
