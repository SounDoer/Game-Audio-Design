...
| 14 | 总结 AIGC 音频资产现状 | AIGC 音频资产的现状 | 从语音、音乐、音效三类分别总结各自的使用方式和主要边界 | 三分栏 v-click bullet | three-column summary | 作为语音、音乐、音效后的横向总结页，帮助学生建立分类判断 | confirmed |
| 15 | 补充风险演进 | 这些问题正在如何演进？ | 配音演员授权、知情同意、补偿；Suno 诉讼、和解、授权模型 | `slides/public/ai-in-game-audio-design-2026/cmu-suno-legal-post.png` | trend / news | 不是法律课，只提醒“可商用”背后有制度变化 | confirmed |
...
| 17 | AI 工具一 | 素材发现与管理：从找文件到找意图 | 左栏：文件名、关键词、人工标签、素材库经验、反复试听；右栏：语义搜索、相似声音搜索、自动标签、智能素材库整理，Soundly / SoundQ 与 Orion / SFX Stacks 作为代表方向 | 双栏文字对照，无需截图 | before-after | 将原有工作流与 AI powered 新思路 / 新工具统一到一页 | confirmed |
| 18 | AI 工具二 | 音频编辑与修复 | 左栏：人工听辨、spectral editing、插件链、经验参数、时间与质量取舍；右栏：iZotope RX、Adobe Enhance Speech、stem separation，强调 AI 先处理清理、分离和修复的重复劳动，人仍要判断是否可用、可交付 | 双栏文字对照，无需截图 | before-after | 将传统修复流程与 AI powered 修复方向统一到一页 | confirmed |
| 19 | AI 工具三 | 技术音频与实现辅助 | 左栏：手写脚本、查 API、重复对象和命名规则、Wwise / Unreal / Unity 中反复设置和排查、实现验证；右栏：WAAPI、Wwise MCP、UE Audio MCP、Unity / Unreal AI、ReAgent，强调从成熟自动化到 agent 化工作流 | 双栏文字对照，无需截图 | before-after | 将传统技术音频实现流程与 AI powered 实现辅助方向统一到一页 | confirmed |
| 20 | 自定义工具示例 | 自定义工具：Reaper to Wwise | 左栏展示 `slides/public/ai-in-game-audio-design-2026/reaper-to-wwise.png`；右侧小区域由用户补充说明 | `slides/public/ai-in-game-audio-design-2026/reaper-to-wwise.png` | image + side note | 用作者自己的小工具承接技术音频与实现辅助主题 | placeholder |
| 21 | Section 1 收束 | 游戏公司内部正在如何布局？ | 纯 statement，用问题从行业工具地图过渡到下一段 | statement layout | statement | 放在自定义工具案例之后，作为转入后续内容的提问页 | confirmed |
| 22 | Section 2 开场 | 关于如何使用 AI，大家其实是站在同一起跑线上的。 | 不是记住工具名，而是建立协作方式和判断标准 | 纯文字 | statement | 从行业观察转入个人经验 | confirmed |
| 23 | 阶段一案例 | 从需求开始：为什么我要做 audiometer？ | 个人听声习惯：用监测表头观察桌面端各种音频信号，包括视频、游戏和开发中场景；现有工具无法同时满足 LUFS 响度和频谱观察需求；因此开始做自己的 audiometer | `slides/public/how-to-listen-sound/Youlean-Loudness-Meter.png`；`slides/public/how-to-listen-sound/TC-CLARITY-M-STEREO.png` | case-story | 从真实听声习惯切入：不是为了写代码而写代码，而是因为现有工具不够贴合自己的工作方式 | confirmed |
| 24 | 个人背景 | 我不是零基础，但过去仍被门槛卡住 | 三列：版本控制工具、代码编辑工具、以往案例；工具名链接到 GitHub、Perforce、VS Code、JetBrains Rider 官网；以往案例为个人网站、Game Jam Demo、在线 Slides | `slides/public/ai-in-game-audio-design-2026/github-soundoer.png`；`slides/public/ai-in-game-audio-design-2026/perforce-localtts.png`；`slides/public/ai-in-game-audio-design-2026/vscode-plvs.png`；`slides/public/ai-in-game-audio-design-2026/jetbrains-rider.webp` | three-column context | 背景要短，不抢主线 | confirmed |
| 25 | 阶段一体验 | 想法第一次能落地 | 左侧展示 `slides/public/ai-in-game-audio-design-2026/audiometer-legacy.png`；右侧用 bullet 说明早期 vibe coding 的问题：想到什么做什么、缺少检验标准、缺少统一界面规范、不知道如何调教 Agents；第四点同时展示 meme | `slides/public/ai-in-game-audio-design-2026/audiometer-legacy.png`；`slides/public/ai-in-game-audio-design-2026/vibe-coding-meme.jfif` | image + side note | 重点是把早期兴奋感和失控感讲清楚 | confirmed |
| 26 | 阶段二 | 从聊天，到理解 agents 与 skills | 两块内容：coding agents 不是简单的 LLM Chatbot，需要理解 Rule、Skill、Subagents、Hook、MCP 等术语和工具能力边界；术语链接使用 AGENTS.md / Agent Skills / Codex Subagents / Claude Agent SDK Hooks / MCP 官方文档；右侧介绍 Agent Skills 的定义和标准目录结构 | 可用 skills 分类图 | process / list | 讲 agents 和 skills 是流程设计，不是工具清单 | confirmed |
| 26a | 补充协作原则案例 | Karpathy-Inspired Guidelines | eyebrow 中仅显示 GitHub stars badge；左侧合并说明 `andrej-karpathy-skills` 的来源与四条原则；右侧展示完整 `CLAUDE.md` guideline 原文 | 官方中文 README：https://github.com/multica-ai/andrej-karpathy-skills/blob/main/README.zh.md；GitHub stars badge：https://img.shields.io/github/stars/multica-ai/andrej-karpathy-skills?style=social；右侧代码块为用户提供的 `CLAUDE.md` 内容 | principle / text + full markdown source | 放在理解 agents 与 skills 之后，用它说明真正的改进来自协作协议和验收标准，而不只是换模型 | confirmed |
| 26b | 补充 skills repo 案例 | Skills For Real Engineers | eyebrow 中显示 `mattpocock/skills` GitHub stars badge；左侧说明真实应用开发很难，大流程框架会接管过程但降低人的控制感，而 `mattpocock/skills` 把工程经验拆成小型、容易改造、可组合、适用于任何模型的 skills；右侧展示 `grill-me` skill 内容 | GitHub repo：https://github.com/mattpocock/skills；GitHub stars badge：https://img.shields.io/github/stars/mattpocock/skills?style=social；右侧代码框为用户提供的 `grill-me` 内容 | principle / text + skill source | 放在 Karpathy Guidelines 之后，继续说明 skills 如何把“先问清楚、再行动”的协作方式流程化 | confirmed |
| 27 | 阶段三 | 协作范式的 before / after | 以前：我描述功能，AI 写代码；现在：我定义目标、边界、标准、验收，AI 执行、检查、迭代 | callback audiometer | before-after | 回到 audiometer 后续功能开发 | confirmed |
| 28 | 规范化开发意识 | 为什么要有这些文档？ | 设计系统、design token、组件规范、验收标准；解决 AI 断片和 SSOT 问题 | 可用文档/SSOT 示意 | concept / diagram | 继续围绕 audiometer 展开 | confirmed |
| 29 | Audiometer 成品展示 | PLVS：这个工具现在变成了什么？ | PLVS 是一个实时音频监测桌面工具；展示官网、GitHub、实际工具截图，以及它如何从个人听声需求变成可发布的开源产品 | TODO(user): 工具实际截图；链接：https://plvs.soundoer.com/ 与 https://github.com/SounDoer/PLVS | showcase | 可以简短点出 Peak、Loudness、Vectorscope、Spectrum、Spectrogram，以及系统音频捕获、灵活布局、历史快照等代表功能；重点仍是“需求如何变成产品形态” | confirmed |
| 30 | 人在 AI 协作中的价值 | 工程决策 vs 审美决策 | 工程：目标、边界、架构、验收；审美：风格、取舍、体验、品味 | 双栏图 | two-column | 这是个人经验的观点收束 | confirmed |
| 31 | 彩蛋开场 | 这套 slides 也是 AI 协作完成的 | 不只代码可以和 AI 协作，课程准备、内容结构和 slides 实现也可以被流程化 | TODO(user): Slidev 全览图 | meta / overview | 用全览图把观众带回这套 deck 本身：前面讲的方法，正在被这套 slides 使用 | confirmed |
| 32 | 讲解 slides authoring skill | `gad-slides-authoring`：把协作拆成 A / B / C | A 先决定讲什么；B 决定每页怎么讲；C 才决定怎么做成 Slidev。A 大纲：确定主线和结构；B 逐页内容：确认每页目的、文字、媒体和 layout intent；C Slidev 实现：根据 confirmed page plan 写 `slides/pages/<stem>.md` 并构建验证 | 可用 `gad-slides-authoring/SKILL.md` 截图或三阶段流程图 | process / diagram | 按“内容生产流水线”讲：skill 的价值不是“让 AI 自由发挥”，而是用阶段门槛、placeholder 和 confirmed page plan 控制协作边界 | confirmed |
| 33 | 讲解 Slidev 设计 SSOT | `README.md` 与 `EXAMPLE.md`：规则说明书 + 可复制样张 | `slides/README.md` 是规则说明书，回答“这个 deck 系统怎么运转”：目录、构建、设计 token、layout 体系和资源约定；`slides/pages/EXAMPLE.md` 是可复制样张，回答“具体页面应该怎么写”：cover、section、header-body、fullscreen-media、statement 等标准写法 | 可用 `slides/README.md` 与 `slides/pages/EXAMPLE.md` 截图 | docs / reference | 强调 SSOT 不只是文档，而是“规则 + 样例”；这些文件让 AI 和人使用同一套设计语言，减少断片、风格漂移和临时写法 | confirmed |
| 34 | 结尾 statement | 探索你自己的协作方式，解决你自己的实际需求。 | “AI 时代，只要你学得足够慢，你就可以什么都不用学。” 🤣 点击出现 | 纯文字 | statement | 全课收束，从设计师的判断责任转为学习者的行动建议 | confirmed |
...
| Reaper to Wwise 自定义工具截图 | `slides/public/ai-in-game-audio-design-2026/reaper-to-wwise.png` |  | ready |
| Audiometer 早期案例素材 | `slides/public/how-to-listen-sound/Youlean-Loudness-Meter.png`；`slides/public/how-to-listen-sound/TC-CLARITY-M-STEREO.png` |  | ready |
| 个人开发背景截图 | `slides/public/ai-in-game-audio-design-2026/github-soundoer.png`；`slides/public/ai-in-game-audio-design-2026/perforce-localtts.png`；`slides/public/ai-in-game-audio-design-2026/vscode-plvs.png`；`slides/public/ai-in-game-audio-design-2026/jetbrains-rider.webp` | TODO(user): 代码相关案例图片 | partial |
