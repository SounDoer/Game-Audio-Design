...
* [明确设计需求](#明确设计需求)
* [尝试使用 Wwise Trigger 功能](#尝试使用-wwise-trigger-功能)
* [那就自己动手写吧](#那就自己动手写吧)
	* [获取当前播放位置信息](#获取当前播放位置信息)
	* [获取节拍点位置信息](#获取节拍点位置信息)
	* [计算函数调用时间点](#计算函数调用时间点)
* [最终实现](#最终实现)
* [总结](#总结)
...
![HandleAttack Function Explanation](HandleAttack.jpeg)
...
![Wwise Trigger Solution](WwiseTriggerSolution.jpeg)
...
![Wwise Trigger Solution Test](WwiseTriggerSolution_Test.png)
...
![Wwise SDK PostTrigger](AkSoundEngine_PostTrigger.png)

至此，尝试使用 Wwise Trigger 功能来实现“嵌套式”的 Custom Cue 触发回调函数的方式验证不可行。

## 那就自己动手写吧

![Position On Track](PositionOnTrack.jpeg)
...
![Wwise SDK GetSourcePlayPosition](AkSoundEngine_GetSourcePlayPosition.png)
...
![PostEvent Callback Mask](Blueprint_PostEventCallbackMask.png)
...
![MusicCue Data Table](MusicCueDataTable.png)
...
![MusicSegment Data Table](MusicSegmentDataTable.png)
...
![Final UE Blueprint Implementation](BlueprintImplementation.png)

如上图，在 Blueprint 中简单使用几个节点即可实现最终的设计需求。

下图为整体的实现思路概览。

![Final Solution Overview](SolutionOverview.jpeg)
