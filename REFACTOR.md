...
- [ ] **弱化「在线课程」产品感**：首页当前仍以 **Course / Topic 分组** 为主；后续可评估是否演进为统一文章流（如时间线）或标签视图。
...
- [ ] **首页与信息架构**：首页已改为基于 `docs/` 路径的 **自动目录树**（Course / Topic 可折叠层级）；是否增加 **标签** 或统一文章流，按需排期。
...
| 6 | **邮件订阅**（二期或占位） | ⏳ 未开始 |
| — | **可选增强**：按篇 **`og:image`**；首页信息架构继续演进（标签/统一文章流）；**`Course.md`** 欢迎页是否恢复 | 🔜 按需 |
